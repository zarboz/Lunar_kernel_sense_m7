...
static struct dsi_buf t6_panel_tx_buf;
static struct dsi_buf t6_panel_rx_buf;
...
void t6_set_cabc (struct msm_fb_data_type *mfd, int mode);
#endif
...
static int __devinit t6_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
...
static void t6_display_on(struct msm_fb_data_type *mfd)
{
...
static void t6_display_off(struct msm_fb_data_type *mfd)
{
...
static struct msm_fb_panel_data t6_panel_data = {
	...
	.display_on = t6_display_on,
	.display_off = t6_display_off,
...
static int mipi_t6_device_register(struct msm_panel_info *pinfo,
...
static int __init t6_panel_init(void)
{
	if(panel_type == PANEL_ID_NONE)	{
	...
	mipi_dsi_buf_alloc(&t6_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&t6_panel_rx_buf, DSI_BUF_SIZE);
