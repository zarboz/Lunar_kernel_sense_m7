...
int t6china_main_camera_vreg_on(void)
{
    ...
    pr_info ("%s: t6china_main_camera_on=%d\n",__func__,t6china_main_camera_on);

    if (t6china_main_camera_on==1) {
        
        rc = camera_sensor_power_enable("8921_l29", 2850000, &reg_8921_l29);
        if (rc<0)
            goto FAIL_VCM_POWER;
...
void t6china_yushanii_probed_as0260(enum htc_camera_image_type_board htc_image)
{
    ...
    msm_camera_sensor_as0260_data.video_hdr_capability &= msm_camera_sensor_as0260_data.htc_image;
}
